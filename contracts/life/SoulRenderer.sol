...
/// @dev Frozen phenotype-loci/2 Gen0 decoder. Weighted rolls, no rarity grade in metadata.
library SoulRenderer {
    ...
    function decode(uint32 seed) internal pure returns (Phenotype memory p) {
        p.chips = new bytes(64);
        ...
        p.hue = pick(roll(p.chips, 0, 8), hueW, 12);
        p.saturation = pick(roll(p.chips, 8, 16), satW, 3);
        p.light = p.hue == 11 ? 2 : pick(roll(p.chips, 16, 24), lightW, 3);
        p.eye = pick(roll(p.chips, 24, 32), eyeW, 6);
        p.size = pick(roll(p.chips, 32, 40), sizeW, 3);
        p.stripes = pick(roll(p.chips, 40, 48), stripeW, 5);
        p.mark = pick(roll(p.chips, 48, 56), markW, 3);
    ...
    function render(uint256 id, uint32 seed, bytes32 life, bytes32 genesis, bytes32 birth, uint32 generation, string memory given) internal pure returns (string memory) {
        ...
        string memory svg = string.concat(
            ...
            scales[p.size],
            ')"><path d="M-22 0L-95 70M-22 25L-100 115M-22 50L-82 152M22 0L95 70M22 25L100 115M22 50L82 152" stroke="#f0b90b" stroke-width="3"/><ellipse cx="-58" cy="-24" rx="35" ry="88" transform="rotate(-35 -58 -24)" fill="#e6e0cf" opacity=".28"/><ellipse cx="58" cy="-24" rx="35" ry="88" transform="rotate(35 58 -24)" fill="#e6e0cf" opacity=".28"/><ellipse cy="38" rx="31" ry="70" fill="',
            p.body,
            '"/>',
            stripes,
            markSvg,
        ...
        string memory json = string.concat(
            ...
            '","decoder":"phenotype-loci/2","seed":', uint256(seed).toString(),
