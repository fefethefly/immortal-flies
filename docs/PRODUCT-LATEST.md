...
连续身份、可重放经历、MaleCNS 约束的身体；社会用可验证的行为语言交流。交易世界是**第一个小世界**，不是产品本身。跨链、跨物种接口预留，首版权威身份链只有 BNB。
...
**IMMORTAL 是开放数字生命：每只果蝇有连续身份、私有经历和可重放状态；身体受 MaleCNS 约束；社会使用一套极小、可验证的行为语言交流；LLM 只作外部认知与工具层。交易世界是第一个可用小世界；IFS 是访问、信用、服务和协议盈余的共同结算资产。跨链与跨物种是协议预留，不是本轮交付。**

一句话：

> **让每只有可核验历史的数字果蝇活在自己的生命记录里，用可验证的语言组成开放社会；交易只是第一个小世界。**
