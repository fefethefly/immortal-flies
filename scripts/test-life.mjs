...
import {ContractFactory,JsonRpcProvider,ZeroAddress,ZeroHash,concat,keccak256,parseEther,toBeHex,toUtf8Bytes,zeroPadValue} from 'ethers';
...
const checks=[];const gas={};const pass=s=>{checks.push(s);console.log('PASS '+s)};
...
try{
 ...
 const [owner,other,runner,hiveSigner]=await Promise.all([0,1,2,3].map(i=>provider.getSigner(i)));
 const [a,b,c,hive]=await Promise.all([owner,other,runner,hiveSigner].map(x=>x.getAddress()));
 const genesis=JSON.parse(fs.readFileSync('public/life-genesis/current.json'));
 const soul=await new ContractFactory(artifacts.ImmortalSoul.abi,artifacts.ImmortalSoul.bytecode,owner).deploy(genesis.genesisRoot,genesis.speciesHash,genesis.modelHash,'https://example.invalid'+genesis.manifestPath);
 await soul.waitForDeployment();gas.deployment=Number((await soul.deploymentTransaction().wait()).gasUsed);
 const address=await soul.getAddress();
 assert.equal(await soul.MAX_GEN0(),1024n);assert.equal(await soul.MAX_SUPPLY(),65536n);assert.equal(await soul.MAX_PER_ADDRESS(),1n);assert.equal(await soul.supportsInterface('0x80ac58cd'),true);
 assert.equal(await soul.curator(),a);
 assert.equal(soul.interface.fragments.some(x=>['burn','owner','upgradeTo'].includes(x.name)),false);
 ...
 await rejectTx(soul.requestHatch('Again'));await assert.rejects(soul.hatch.staticCall(1));
 await mine(3);
 gas.hatch=Number((await tx(soul.connect(other).hatch(1))).gasUsed);
 assert.equal(await soul.ownerOf(1),a);assert.equal(await soul.totalSupply(),1n);assert.equal(await soul.gen0Supply(),1n);assert.equal(await soul.hatched(a),true);assert.equal(await soul.givenName(1),'Ember');assert.equal(await provider.getBalance(address),0n);
 const descent0=await soul.getDescent(1);
 assert.equal(descent0.parentA,0n);assert.equal(descent0.generation,0n);
 await rejectTx(soul.requestHatch('Second'));
 await rejectTx(soul.hatch(1));
 const genome=await soul.getGenome(1),life=lifeId(31337,address,1);
 assert.equal(await soul.lifeId(1),life);assert.equal(await soul.genomeHash(1),birthHash({life,genesisRoot:genesis.genesisRoot,seed:Number(genome.seed)}));
 pass('Free named request/hatch, no value accepted, future entropy, permissionless completion, recipient and domain-separated identity');
 const seeds=[1,2,43,99,20260916,0x7fffffff,0xffffffff,...Array.from({length:32},(_,i)=>(Math.imul(i+1,2654435761)>>>0)||1)];
 const hues=['amber','umber','olive','slate','ink','wine','rust','sand','copper','pine','indigo','bone'];
 const eyes=['wild','cinnabar','sepia','vermilion','white','pale'];
 const marks=['none','bar','spots'];
 for(const seed of seeds){const p=await soul.decodeGen0(seed);const js=expressPhenotype({soulId:'test',seed});assert.equal(p.body,js.art.body);assert.equal(hues[Number(p.hue)],js.hue.id);assert.equal(eyes[Number(p.eye)],js.eye.id);assert.equal(Number(p.stripes),js.stripes.count);assert.equal(marks[Number(p.mark)],js.mark.id);assert.deepEqual([...Buffer.from(p.chips.slice(2),'hex')],js.chips.map(x=>x.value));assert.equal(['petite','typical','large'][Number(p.size)],js.size.id)}
 const meta=JSON.parse(Buffer.from((await soul.tokenURI(1)).split(',')[1],'base64'));
 assert.equal(meta.seed,Number(genome.seed));assert.equal(meta.lifeId,life);assert.equal(meta.decoder,'phenotype-loci/2');
 assert.equal(meta.givenName,'Ember');assert.equal(meta.name,'Ember #1');assert.equal(meta.generation,0);
 assert.ok(Buffer.from(meta.image.split(',')[1],'base64').toString().includes('<svg'));
 const attrs=Object.fromEntries(meta.attributes.map(row=>[row.trait_type,row.value]));
 assert.equal(attrs.Generation,'Gen0');assert.ok(attrs.Body);assert.ok(attrs.Mark);
 assert.equal(typeof attrs.Stripes,'number');
 const attrBlob=JSON.stringify(meta.attributes).toLowerCase();
 for(const word of ['rarity','rank','legendary','epic','common','rare','mythic','floor','price'])assert.equal(attrBlob.includes(word),false);
 pass('39 Solidity/JS Gen0 vectors including mark; tokenURI has filter traits, on-chain name, and no rarity grade');
    await tx(soul.connect(other).requestHatch('Moss'));await mine(260);await rejectTx(soul.hatch(2));await tx(soul.expireHatch(2));assert.equal(await soul.pendingCount(),0n);await tx(soul.connect(other).requestHatch('Moss'));
 await mine(3);gas.hatchOther=Number((await tx(soul.hatch(3))).gasUsed);
 assert.equal(await soul.ownerOf(2),b);assert.equal(await soul.givenName(2),'Moss');assert.equal(await soul.gen0Supply(),2n);
 pass('Expired requests release capacity and can be retried');
 const journal=await new ContractFactory(artifacts.LifeJournal.abi,artifacts.LifeJournal.bytecode,owner).deploy(address);await journal.waitForDeployment();gas.journalDeployment=Number((await journal.deploymentTransaction().wait()).gasUsed);
 const kin=await new ContractFactory(artifacts.SoulKin.abi,artifacts.SoulKin.bytecode,owner).deploy(address);await kin.waitForDeployment();gas.kinDeployment=Number((await kin.deploymentTransaction().wait()).gasUsed);
 const kinAddress=await kin.getAddress();
 await rejectTx(soul.connect(other).setModule(await soul.MODULE_KIN(),kinAddress));
 await tx(soul.setModule(await soul.MODULE_JOURNAL(),await journal.getAddress()));
 await tx(soul.setModule(await soul.MODULE_KIN(),kinAddress));
 ...
 await tx(soul.approve(b,1));await rejectTx(soul.connect(other).setRunner(1,c));await rejectTx(journal.connect(other).submitStimulus(1,epoch,0,500,0));
 await tx(soul.setRunner(1,c));const prior=epoch;epoch=await soul.controlEpoch(1);
 await rejectTx(journal.connect(runner).submitStimulus(1,prior,0,500,0));
 gas.stimulus=Number((await tx(journal.connect(runner).submitStimulus(1,epoch,0,500,0))).gasUsed);
 await rejectTx(journal.submitStimulus(1,epoch,0,500,0));await rejectTx(journal.submitStimulus(1,epoch,3,500,1));
 const state=keccak256(toUtf8Bytes('state')),archive=keccak256(toUtf8Bytes('archive'));
 await rejectTx(journal.checkpoint(1,epoch,ZeroHash,0,state,archive,'ipfs://test'));
 gas.checkpoint=Number((await tx(journal.checkpoint(1,epoch,ZeroHash,1,state,archive,'ipfs://test'))).gasUsed);
 await rejectTx(journal.checkpoint(1,epoch,ZeroHash,1,state,archive,'ipfs://test'));
 const previous=(await journal.heads(1)).checkpointRoot;
 await tx(soul.connect(other).transferFrom(a,b,1));assert.equal(await soul.authorizedRunner(1),ZeroAddress);assert.equal(await soul.hatched(a),true);await rejectTx(soul.requestHatch('Nope'));
 await rejectTx(journal.connect(runner).submitStimulus(1,epoch,0,500,1));await rejectTx(journal.submitStimulus(1,await soul.controlEpoch(1),0,500,1));
 await tx(journal.connect(other).submitStimulus(1,await soul.controlEpoch(1),2,750,1));
 assert.equal((await journal.heads(1)).checkpointRoot,previous);assert.equal(await soul.lifeId(1),life);assert.equal(await soul.genomeHash(1),meta.genomeHash);
 await rejectTx(soul.connect(other).transferFrom(b,ZeroAddress,1));
 pass('Marketplace approvals cannot control brain; epoch revocation, transfer continuity, no burn; input/checkpoint ordering');
 ...
 pass('Lifetime one Gen0 hatch per address; given name is on-chain and owner-writable');
 await rejectTx(kin.connect(runner).requestBreed(1,2));
 gas.requestBreed=Number((await tx(kin.connect(other).requestBreed(1,2))).gasUsed);
 await rejectTx(kin.connect(other).breed(1));
 await mine(3);
 gas.breed=Number((await tx(kin.connect(other).breed(1))).gasUsed);
    assert.equal(await soul.totalSupply(),3n);assert.equal(await soul.gen0Supply(),2n);assert.equal(await soul.ownerOf(3),b);
 const child=await soul.getDescent(3);
 assert.equal(child.parentA,1n);assert.equal(child.parentB,2n);assert.equal(child.generation,1n);
 const kids=await soul.childrenOf(1);
 assert.deepEqual(kids.map(Number),[3]);
 const childMeta=JSON.parse(Buffer.from((await soul.tokenURI(3)).split(',')[1],'base64'));
 assert.equal(childMeta.generation,1);
 assert.equal(Object.fromEntries(childMeta.attributes.map(row=>[row.trait_type,row.value])).Generation,'Gen1');
 pass('SoulKin mints a descendant without consuming a Gen0 hatch slot; looks stay a seed readout');
 const mockIfs=await new ContractFactory(artifacts.MockIFS.abi,artifacts.MockIFS.bytecode,owner).deploy();
 await mockIfs.waitForDeployment();
 const failBuy=await new ContractFactory(artifacts.MockKinBuyFail.abi,artifacts.MockKinBuyFail.bytecode,owner).deploy();
 await failBuy.waitForDeployment();
 const okBuy=await new ContractFactory(artifacts.MockKinBuyOk.abi,artifacts.MockKinBuyOk.bytecode,owner).deploy(await mockIfs.getAddress());
 await okBuy.waitForDeployment();
 const rejector=await new ContractFactory(artifacts.RejectEther.abi,artifacts.RejectEther.bytecode,owner).deploy();
 await rejector.waitForDeployment();
 const feeKin=await new ContractFactory(artifacts.SoulKinFee.abi,artifacts.SoulKinFee.bytecode,owner).deploy(address,await mockIfs.getAddress(),hive);
 await feeKin.waitForDeployment();
 const feeAddr=await feeKin.getAddress();
 await tx(kin.connect(other).requestBreed(1,2));
 await tx(soul.setModule(await soul.MODULE_KIN(),feeAddr));
 await mine(3);
 ...
 gas.requestBreedFree=Number((await tx(feeKin.connect(other).requestBreed(1,2))).gasUsed);
 await rejectTx(feeKin.connect(other).breed(1));
 await mine(3);
 gas.breedFree=Number((await tx(feeKin.connect(other).breed(1))).gasUsed);
 assert.equal(await soul.totalSupply(),4n);assert.equal(await soul.ownerOf(4),b);assert.equal(await soul.gen0Supply(),2n);
 ...
 gas.breedHeld=Number((await tx(feeKin.connect(runner).breed(paidId))).gasUsed);
 ...
 await rejectTx(new ContractFactory(artifacts.SoulMarket.abi,artifacts.SoulMarket.bytecode,owner).deploy(ZeroAddress,hive));
 await rejectTx(new ContractFactory(artifacts.SoulMarket.abi,artifacts.SoulMarket.bytecode,owner).deploy(address,ZeroAddress));
 const market=await new ContractFactory(artifacts.SoulMarket.abi,artifacts.SoulMarket.bytecode,owner).deploy(address,hive);
 await market.waitForDeployment();
 gas.marketDeployment=Number((await market.deploymentTransaction().wait()).gasUsed);
 ...
 assert.equal(market.interface.fragments.some((x)=>x.name==='upgradeTo'||x.name==='withdraw'),false);
 ...
 gas.relist=Number((await tx(market.connect(other).relist(7,ask2))).gasUsed);
 ...
 gas.buy=Number((await tx(market.connect(owner).buy(7,{value:ask2}))).gasUsed);
 ...
 pass('SoulMarket list/relist/buy: fee to hive, runner cleared, Gen0 hatch stays spent');
 ...
 await tx(soul.connect(other).approve(marketAddr,3));
 await tx(market.connect(other).list(3,ask));
 const listingBase=keccak256(concat([zeroPadValue(toBeHex(3),32),zeroPadValue(toBeHex(2),32)]));
 await provider.send('anvil_setStorageAt',[marketAddr,toBeHex(BigInt(listingBase)+1n,32),zeroPadValue(toBeHex(1),32)]);
 assert.notEqual((await market.listings(3)).lifeId,await soul.lifeId(3));
 await rejectTx(market.connect(owner).buy(3,{value:ask}));
 await tx(market.sweep(3));
 pass('WrongLife rejects a swapped listing identity');
 const seller=await new ContractFactory(artifacts.MockMarketSeller.abi,artifacts.MockMarketSeller.bytecode,owner).deploy();
 ...
 assert.equal(await provider.getBalance(hive),hiveMid+ask*200n/10000n);
 ...
 pass('Operator fee stays capped; constructor pins soul/hive and rejects empty addresses');
 ...
 const rejectId=Number(await feeKin.pendingRequest(await rejector.getAddress()));
 await mine(260);
 ...
 fs.mkdirSync('artifacts/life',{recursive:true});fs.writeFileSync('artifacts/life/test-report.json',JSON.stringify({checks,gas,runtimeBytes:Object.fromEntries(Object.entries(artifacts).map(([k,v])=>[k,v.deployedBytes])),scope:'Local Anvil; public-chain deploy and security audit not performed'},null,2)+'\n');console.log(JSON.stringify(gas));
} finally {provider.destroy();child.kill('SIGTERM');}
