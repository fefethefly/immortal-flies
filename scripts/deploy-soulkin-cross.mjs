/**
 * Deploy SoulKinCrossFee (crossover descent) against an already-live ImmortalSoul.
 *
 * 交叉规则：每个位点取自一位亲本，约 2% 突变；完成者在链下研磨全中候选，
 * breed(id, n) 链上一次解码验证。替换 MODULE_KIN 即启用，不动 Soul。
 *
 * Testnet:
 *   npm run life:check:kin-cross
 *   IFF_DEPLOY_KEY=0x… npm run life:deploy:kin-cross:testnet
 *   IFF_DEPLOY_KEY=0x… npm run life:deploy:kin-cross:testnet -- --bind
 *
 * Mainnet deploy of the satellite only (does not touch Soul):
 *   npm run life:check:kin-cross -- --mainnet
 *   IFF_DEPLOY_KEY=0x… npm run life:deploy:kin-cross:testnet -- --mainnet
 *
...
function readKey() {
  ...
  if (!/^0x[0-9a-fA-F]{64}$/.test(key)) {
    throw new Error(
      "缺少 IFF_DEPLOY_KEY。可放在项目 .env 或 ~/.env，不要提交。",
    );
  }
  return key;
}
...
console.log(`ifs ${ifs}`);
console.log(`SoulKinCrossFee ${artifacts.SoulKinCrossFee.deployedBytes}B`);
...
if (bind) {
  const oldKin = new Contract(deployment.kin, artifacts.SoulKin.abi, provider);
...
const record = {
  ...
  kinFee: feeAddr,
  kinKind: bind ? "SoulKinCrossFee" : deployment.kinKind || "SoulKin",
  ifs,
  hive: hiveAddr,
  ...
  note: bind
    ? "SoulKinCrossFee is MODULE_KIN. Price starts at 0, adapter unset. Not a burn. Do not redeploy ImmortalSoul."
    : "SoulKinCrossFee deployed but not bound. Current kin is unchanged.",
};
const out = fs.existsSync(listing) ? listing : fallback;
if (mainnet && !bind) {
  const satellite = path.join(
    root,
    "public/contract/life/SoulKinCrossFee.deployment.json",
  );
