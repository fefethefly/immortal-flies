...
// Schematic holographic artwork. Projection sweeps and internal light paths are
// decorative, and deliberately separate from the observatory's measured graph.
export function createHologramScene(canvas, read, onFailure) {
  ...
  const shell = new THREE.ShaderMaterial({
    ...
    fragmentShader: `uniform float time;uniform vec3 color;uniform vec3 amber;
      ...
      void main(){float rim=pow(1.-abs(dot(normalize(vNormal),normalize(vView))),2.5);
        ...
        gl_FragColor=vec4(light,(.022+rim*.35+wire*.105+sweep*.26)*bands);}`,
    transparent: true,
    depthWrite: false,
  ...
  function draw() {
    if (dead) return;
    const still = read().paused || read().reduced;
    tiltX += (pointerX - tiltX) * 0.06;
    tiltY += (pointerY - tiltY) * 0.06;
    specimen.rotation.set(
      0.36 + (still ? 0 : tiltY * 0.16),
      -0.26 + (still ? 0 : tiltX * 0.38 + Math.sin(time * 0.2) * 0.08),
      -0.28,
    );
    specimen.position.y = 0.1 + (still ? 0 : Math.sin(time * 0.72) * 0.025);
    wings.forEach((wing, i) => {
      const sign = i === 0 ? -1 : 1;
      wing.rotation.y =
        sign * (0.16 + (still ? 0 : Math.sin(time * 2.5) * 0.025));
    });
