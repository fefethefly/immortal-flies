...
const store = {
  async db() {
    return new Promise((resolve, reject) => {
      const r = indexedDB.open("iff-connectome", 1);
      r.onupgradeneeded = () => r.result.createObjectStore("archives");
  ...
  async get(key) {
    ...
    try {
      return await new Promise((resolve, reject) => {
        const r = db.transaction("archives").objectStore("archives").get(key);
        r.onsuccess = () => resolve(r.result);
        r.onerror = () => reject(r.error);
      });
    } finally {
  ...
  async put(key, data) {
    ...
    try {
      return await new Promise((resolve, reject) => {
        ...
        tx.objectStore("archives").put(data, key);
...
function groupColors(groups) {
  const map = new Map();
  for (const [name, , color] of SIGNAL_GROUPS)
    for (const i of groups?.[name] || []) map.set(i, color);
  return map;
}
...
function NeuralScene({ nodes, edges, state, bodyView, groups }) {
  ...
  }, [nodes, edges, state, bodyView, groups, tx]);
  useEffect(() => {
    ...
    function draw() {
      ...
      const { nodes, edges, state, bodyView, groups, tx } = latest.current;
      if (!nodes.length) return;
      const role = groups instanceof Map ? groups : new Map();
      const cx = width / 2,
        cy = height / 2;
      ctx.strokeStyle = "#2f2a1f";
      ctx.lineWidth = 0.7;
      for (let i = 0; i < 12; i++) {
        const y = height * 0.18 + i * height * 0.07;
        ctx.beginPath();
        ctx.moveTo(0, y);
        ctx.lineTo(width, y);
        ctx.stroke();
      ...
      for (let i = -8; i < 9; i++) {
        ctx.beginPath();
        ctx.moveTo(cx + i * 28, height * 0.13);
        ctx.lineTo(cx + i * 100, height);
        ctx.stroke();
      }
      if (bodyView) {
        ctx.strokeStyle = "#8a8172";
        ctx.beginPath();
        ctx.ellipse(cx, cy, width * 0.4, height * 0.34, 0, 0, Math.PI * 2);
        ctx.stroke();
        const x = width * 0.1 + (state.body.x / 10000) * width * 0.8,
        ...
        ctx.fillText(tx("brain.stimulusMark"), foodX + 15, foodY - 12);
        ctx.save();
        ctx.translate(x, y);
        ...
        for (const direction of [-1, 1]) {
          ...
          for (let j = -1; j < 2; j++) {
            ctx.beginPath();
            ctx.moveTo(direction * 4, j * 7);
            ctx.lineTo(direction * 18, j * 12 + 7);
            ctx.stroke();
          }
        }
        ctx.fillStyle = "#f0ead9";
        ctx.beginPath();
        ctx.ellipse(0, 3, 5, 15, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.restore();
        ctx.fillStyle = "#d8c9a4";
        ctx.font = "12px monospace";
        ctx.fillText(tx("brain.act." + state.lastAction), x + 30, y + 30);
        return;
      }
      if (!reduced) angle += 0.001;
      const active = new Set(state?.spikes || []),
        take = Math.min(nodes.length, 1500),
        points = [];
      for (let i = 0; i < take; i++) {
        ...
        points.push([
          cx + x * width * 0.36 * scale,
          cy + p[1] * height * 0.38 * scale,
          scale,
        ]);
      }
  ...
  return (
    <canvas
      ref={ref}
      aria-label={
        bodyView ? tx("brain.ariaSceneBody") : tx("brain.ariaSceneGraph")
      }
    />
  );
}
...
function SignalRack({ state, trace }) {
  ...
  useEffect(() => {
    ...
    for (let i = 1; i < 4; i++) {
      ctx.beginPath();
      ctx.moveTo(0, (cssH * i) / 4);
      ctx.lineTo(cssW, (cssH * i) / 4);
      ctx.stroke();
    }
    ...
    trace.forEach((frame, i) => {
      const x = (i / Math.max(trace.length - 1, 1)) * cssW,
        y = cssH - 6 - ((frame.spikeCount || 0) / max) * (cssH - 16);
      i ? ctx.lineTo(x, y) : ctx.moveTo(x, y);
    });
    ctx.stroke();
    ctx.beginPath();
    ctx.strokeStyle = "#93a181";
    ctx.lineWidth = 1;
    trace.forEach((frame, i) => {
      const preview = frame.voltagePreview || [];
      const mean = preview.length
        ? preview.reduce((a, b) => a + b, 0) / preview.length
        : 0;
      const x = (i / Math.max(trace.length - 1, 1)) * cssW,
        y = cssH / 2 - (mean / 4000) * (cssH / 2 - 8);
      i ? ctx.lineTo(x, y) : ctx.moveTo(x, y);
    });
    ctx.stroke();
  }, [trace]);
  ...
  return (
    <div className="signal-rack" aria-label="本步神经信号">
      <div className="signal-head">
        <span>{tx("brain.signalHead")}</span>
        <small>{tx("brain.signalHint")}</small>
      </div>
      <canvas ref={wave} className="signal-scope" />
      <div className="signal-groups">
        {SIGNAL_GROUPS.map(([key, label, color]) => {
          const row = groups[key] || { size: 0, spikes: 0 };
          const ratio = row.size ? row.spikes / row.size : 0;
          const drive = state?.signal?.[key];
          return (
            <div key={key}>
              <span style={{ color }}>{tx("brain.group." + key)}</span>
              <b>
                {row.spikes} / {row.size}
              </b>
              <i>
              ...
              {drive != null && <small>{drive}</small>}
            </div>
          );
        })}
      </div>
      <div className="signal-hist" aria-hidden="true">
        {(state?.voltageHist || []).map((n, i) => (
          <em
            key={i}
            style={{
              height: `${8 + (n / Math.max(1, ...(state.voltageHist || [1]))) * 28}px`,
            }}
          />
        ))}
      </div>
...
function App() {
  ...
  const [data, setData] = useState(null),
    ...
    [dataset, setDataset] = useState("circuit"),
  ...
  const save = async (silent = false) => {
    const r = await ask({ type: "export" });
    await store.put(dataset, r.result);
    if (!silent) setNotice(tx("brain.notice.saved"));
    return r.result;
  };
  useEffect(() => {
    const w = new Worker(new URL("./brain/worker.mjs", import.meta.url), {
      type: "module",
    });
    worker.current = w;
    w.onmessage = ({ data: r }) => {
    ...
    w.onerror = (e) => {
      setError(e.message || tx("brain.workerErr"));
      setRunning(false);
      for (const p of pending.current.values())
        p.reject(new Error(tx("brain.workerErr")));
  ...
  useEffect(() => {
    fetch("/data/malecns-full/manifest.json")
      .then((r) => setFullReady(r.ok))
  ...
  useEffect(() => {
    ...
    setTrace((prev) => {
      ...
      if (
        last &&
        last.ticks === state.ticks &&
        last.spikeCount === state.spikeCount
      )
        return prev;
      return [
        ...prev,
        {
          ...
          voltagePreview: state.voltagePreview,
        },
      ].slice(-64);
    });
  }, [state]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (!document.hidden)
        act(async () => {
          await ask({ type: "event", event: { type: "step", count: 8 } });
        });
    }, 500);
  ...
  useEffect(() => {
    const persist = () => {
      setRunning(false);
      act(async () => {
        if (worker.current) await save(true);
      });
    };
    const onVisible = () => {
  ...
  const pulse = (adapter) =>
    act(async () => {
      await ask({
        type: "input",
        adapter,
        payload: adapter === "environment" ? input : market,
      });
      await ask({ type: "event", event: { type: "step", count: 32 } });
      setNotice(
        adapter === "environment"
          ? tx("brain.notice.env")
          : tx("brain.notice.marketIn"),
      );
    });
  const download = () =>
    act(async () => {
      ...
      a.href = url;
      a.download = `immortal-${dataset}-${state.ticks}.json`;
      a.click();
  ...
  const importFile = async (e) => {
    ...
    await act(async () => {
      setRunning(false);
      if (file.size > 32 * 1024 * 1024) throw new Error(tx("brain.oversize"));
      const archive = JSON.parse(await file.text());
      await ask({ type: "restore", archive });
      await store.put(dataset, archive);
      setProof(null);
      setNotice(tx("brain.notice.import"));
    });
  };
...
function PanelTitle({ number, title, english }) {
  return (
...
function Toggle({ label, checked, disabled, onChange }) {
  return (
    <button
      className={`source-toggle ${checked ? "on" : ""}`}
      aria-label={label}
      role="switch"
...
function Slider({ label, value, onChange, min = 0, max = 1000 }) {
  return (
    <label className="brain-slider">
      <span>
        {label}
        <b>{value}</b>
      </span>
