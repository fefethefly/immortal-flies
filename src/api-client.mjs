 ...
 */
const BASE = typeof import.meta !== "undefined" && import.meta.env?.VITE_IFF_API_BASE != null
...
async function request(path, { method = "GET", token, body, idempotencyKey, headers = {} } = {}) {
...
export const iffApi = {
  ...
  tick: (id, token, idempotencyKey) =>
    request(`/v1/sessions/${id}/tick`, { method: "POST", token, idempotencyKey }),
  stimulus: (id, token, body) =>
    request(`/v1/sessions/${id}/stimulus`, { method: "POST", token, body }),
  settle: (id, token) => request(`/v1/sessions/${id}/settle`, { method: "POST", token }),
  setRunning: (id, token, running) =>
    request(`/v1/sessions/${id}/running`, { method: "PATCH", token, body: { running } }),
  archive: (id, token) => request(`/v1/sessions/${id}/archive`, { token }),
  restore: (id, token, archive) =>
    request(`/v1/sessions/${id}/restore`, { method: "POST", token, body: { archive } }),
  prove: (id, token) => request(`/v1/sessions/${id}/prove`, { method: "POST", token }),
  ...
  explain: (token, body) => request("/v1/llm/explain", { method: "POST", token, body }),
  ask: (token, body) => request("/v1/llm/ask", { method: "POST", token, body }),
  plan: (token, body) => request("/v1/llm/plan", { method: "POST", token, body }),
  validate: (token, body) => request("/v1/llm/validate", { method: "POST", token, body }),
};
