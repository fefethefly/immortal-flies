...
export const QUORUM_CONFIDENCE_HOLD = Object.freeze({
   ...
   */
  run({ roster, utterances, tick, policy = FLYSWARM_POLICY, era = 0, audit = "SIM" }) {
    ...
    const valid = utterances
      ...
      .sort((a, b) => (a.soulId === b.soulId ? a.sequence - b.sequence : a.soulId < b.soulId ? -1 : 1));
    ...
    for (const u of valid) {
      ...
      const capped = Math.min(base, Math.max(0, policy.voteCapPerRunner - used));
    ...
    if (total > 0) {
      ...
      if (buyOver && sellOver) {
        split = true;
        dissents.push(...votes.map((v) => ({ soulId: v.soulId, side: v.side, weight: v.weight })));
      } else if (buy > sell && buy > hold) {
        side = "BUY";
        dissents.push(...votes.filter((v) => v.side !== "BUY").map((v) => ({ soulId: v.soulId, side: v.side, weight: v.weight })));
      } else if (sell > buy && sell > hold) {
        side = "SELL";
        dissents.push(...votes.filter((v) => v.side !== "SELL").map((v) => ({ soulId: v.soulId, side: v.side, weight: v.weight })));
      } else {
        side = "HOLD";
        dissents.push(...votes.map((v) => ({ soulId: v.soulId, side: v.side, weight: v.weight })));
      }
...
export function createQuorums(additional = []) {
  return createRegistry("quorum", [QUORUM_CONFIDENCE_HOLD, ...additional]);
}
