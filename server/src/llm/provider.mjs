  ...
  async function chat({ messages, tools = null, temperature = 0.2 } = {}) {
    ...
    if (!res.ok) {
      ...
      throw new AppError(
        "LLM_PROVIDER_ERROR",
        502,
        `Provider HTTP ${res.status}: ${text.slice(0, 200)}`,
      );
    }
    const data = await res.json();
    const choice = data.choices?.[0]?.message;
    if (!choice)
      throw new AppError("LLM_PROVIDER_ERROR", 502, "Empty provider response");
    return {
...
export function createFakeProvider({
  narrative = "Local fake model: behavior is rule-explained, not a yield promise.",
} = {}) {
  return {
    configured: true,
    model: "fake-llm-v1",
